...
def draw_panel(
    ...
    selected_connector_style = ""
    if selected_visible_index is not None and 0 <= selected < len(results):
        selected_connector_style = style(fg=2, bold=True) if results[selected].runtime_completion else style(fg=1, bold=True)
